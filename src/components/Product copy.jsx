...
function Product({ product, className, suggestedProduct = false }) {
  ...
  return (
    <Link
      ...
      className={`h-fit block py-9 px-6 shadow-md hover:shadow-lg hover:-translate-y-7 transition-all duration-500 rounded-xl ${
        className || ""
      }`}
    >
      ...
      <div>
        <h3 className="main-title text-xl text-left mt-9">
          {product.title.slice(0, 40) } ....
        </h3>
        <p className="my-3 text-subText">
          {product.description.slice(0, 50) } ....
        </p>
        <p className="mb-1">Price is {product.price}$</p>
